...
@Component({
  selector: 'app-address-form-checkout',
...
export class AddressFormCheckoutComponent {
  form = input.required<FormGroup>();
  countries = input.required<CountrySelect[]>();

  private statusChanges$ = toObservable(this.form).pipe(
    switchMap((form) => form.statusChanges.pipe(startWith(form.status))),
  );

  statusChanges = outputFromObservable(this.statusChanges$);
}
